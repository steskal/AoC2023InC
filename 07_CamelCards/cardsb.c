...
static unsigned short valueHand() {
    ...
    for(unsigned short idx = 0; idx < 5; idx++) {
         ...
         * type.
         * J has valu 1 in the new rule set, but act as a joker that can be 
         * added to any existing combination to make the highest type possible.
         */
        if(figureOutType[idx][0] == 1) countJ = figureOutType[idx][1];
        if(figureOutType[idx][1] == 5) return 6;
        if(figureOutType[idx][1] == 4){ 
            if(figureOutType[idx][0] == 1) 
                return 6; // If there are 4 J you can alwaysadd them to the last 
                          // card as jokers to make 5
            else fourOfKind++; 
    ...
    if(fourOfKind == 1) {
        if(countJ == 0) return 5;
        return 6;
    }
    if(threeOfKind == 1 && twoOfKind == 1) {
        if(countJ == 0) return 4;
        return 6;
    }
    if(threeOfKind == 1) {
        if(countJ == 0) return 3;
        if(countJ > 0) return 5;
    }
    if(twoOfKind == 2) {
        if(countJ == 0) return 2;
        if(countJ == 1) return 4;
        if(countJ == 2) return 5;
    }
    if(twoOfKind == 1) {
        if(countJ == 0) return 1;
        if(countJ == 1) return 3;
        if(countJ == 2) return 3;
    }
    if(countJ == 1) return 1;
